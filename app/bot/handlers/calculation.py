...
async def show_edit_menu(callback: CallbackQuery, state: FSMContext) -> None:
    """Показывает меню редактирования параметров."""
    await safe_answer_callback(callback)
    data = await state.get_data()
    
    await callback.message.answer(
        EDIT_MENU,
        reply_markup=get_edit_menu_keyboard(data),
        parse_mode=ParseMode.HTML
    )
    await state.set_state(CalculationStates.editing_menu)
...
async def recalculate_result(callback: CallbackQuery, state: FSMContext) -> None:
    """Пересчитывает и показывает новый результат."""
    await safe_answer_callback(callback)
    data = await state.get_data()
    ...
    chat_logger.log_message(user_id=user.id, username="БОТ", message=result_text, is_bot=True)
    await _notify_admin(callback.message.bot, user, calculation, data)
...
async def edit_area(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(CalculationStates.editing_area)


@router.message(CalculationStates.editing_area)
async def process_edit_area(message: Message, state: FSMContext) -> None:
    """Обработка ввода новой площади."""
    if not message.text:
        await message.answer(AREA_INVALID_INPUT, parse_mode=ParseMode.HTML)
        return
    ...
    if not validate_range(area, settings.min_area, settings.max_area):
        error = f"❌ Площадь должна быть <b>от {settings.min_area} до {settings.max_area} м²</b>"
...
async def edit_profile(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await send_image_if_exists(
        callback.message, Path("static/images/profiles/profiles_all.jpg")
    )
    await callback.message.answer(
        PROFILE_QUESTION,
    ...
    await state.set_state(CalculationStates.editing_profile)


@router.callback_query(
    CalculationStates.editing_profile,
    F.data.in_(["profile_insert", "profile_shadow", "profile_floating"])
)
async def process_edit_profile(callback: CallbackQuery, state: FSMContext) -> None:
    """Обработка выбора нового профиля."""
    await safe_answer_callback(callback)
    profile_type = callback.data.replace("profile_", "")
    await state.update_data(profile_type=profile_type)
...
async def edit_cornice(callback: CallbackQuery, state: FSMContext) -> None:
    """Редактирование карниза — начинаем с длины."""
    await safe_answer_callback(callback)
    await callback.message.answer(
        CORNICE_LENGTH_QUESTION,
        reply_markup=get_back_keyboard(),
    ...
    await state.set_state(CalculationStates.editing_cornice_length)


@router.message(CalculationStates.editing_cornice_length)
async def process_edit_cornice_length(message: Message, state: FSMContext) -> None:
    """Обработка ввода новой длины карниза."""
    if not message.text:
        await message.answer(CORNICE_INVALID_INPUT, parse_mode=ParseMode.HTML)
        return
    ...
    data = await state.get_data()
    max_length = data.get("area", 50) * 2
    if not validate_range(length, 0, max_length):
        await message.answer(
            get_cornice_validation_error(max_length),
            parse_mode=ParseMode.HTML
        )
        return
    ...
    if length == 0:
        # Без карниза
        await state.update_data(cornice_length=0, cornice_type=None)
        ...
        await state.update_data(cornice_length=length)
        await send_image_if_exists(
            message, Path("static/images/cornices/cornices_all.jpg")
        )
        await message.answer(
            CORNICE_TYPE_QUESTION,
            reply_markup=get_cornice_keyboard(),
            parse_mode=ParseMode.HTML
        )
        await state.set_state(CalculationStates.editing_cornice_type)


@router.callback_query(
    CalculationStates.editing_cornice_type,
    F.data.in_(["cornice_pk5", "cornice_am1", "cornice_pk14", "cornice_bpp", "cornice_bp40", "cornice_none"])
)
async def process_edit_cornice_type(callback: CallbackQuery, state: FSMContext) -> None:
    """Обработка выбора нового типа карниза."""
    await safe_answer_callback(callback)

    if callback.data == "cornice_none":
        await state.update_data(cornice_length=0, cornice_type=None)
        await callback.message.answer(NO_CORNICE, parse_mode=ParseMode.HTML)
    else:
        cornice_type = callback.data.replace("cornice_", "")
        ...
        data = await state.get_data()
        cornice_name = get_cornice_name(cornice_type)
        response = CORNICE_ACCEPTED.format(
            cornice_name=cornice_name,
            length=data.get("cornice_length", 0)
        )
        await callback.message.answer(response, parse_mode=ParseMode.HTML)
...
async def edit_spotlights(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(CalculationStates.editing_spotlights)


@router.message(CalculationStates.editing_spotlights)
async def process_edit_spotlights(message: Message, state: FSMContext) -> None:
    """Обработка ввода нового количества светильников."""
    if not message.text:
        await message.answer(SPOTLIGHTS_INVALID_INPUT, parse_mode=ParseMode.HTML)
        return
...
async def edit_chandeliers(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(CalculationStates.editing_chandeliers)


@router.message(CalculationStates.editing_chandeliers)
async def process_edit_chandeliers(message: Message, state: FSMContext) -> None:
    """Обработка ввода нового количества люстр."""
    if not message.text:
        await message.answer(CHANDELIERS_INVALID_INPUT, parse_mode=ParseMode.HTML)
        return
...
@router.callback_query(CalculationStates.editing_menu, F.data == "go_back")
@router.callback_query(CalculationStates.editing_area, F.data == "go_back")
@router.callback_query(CalculationStates.editing_profile, F.data == "go_back")
@router.callback_query(CalculationStates.editing_cornice_length, F.data == "go_back")
@router.callback_query(CalculationStates.editing_cornice_type, F.data == "go_back")
@router.callback_query(CalculationStates.editing_spotlights, F.data == "go_back")
@router.callback_query(CalculationStates.editing_chandeliers, F.data == "go_back")
async def go_back_to_edit_menu(callback: CallbackQuery, state: FSMContext) -> None:
    """Возврат в меню редактирования из любого состояния редактирования."""
    await safe_answer_callback(callback)
    data = await state.get_data()
    await callback.message.answer(
        EDIT_MENU,
